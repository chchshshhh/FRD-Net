...
# from debmodule.diffconv.diffconv import Conv2d_cd
from debmodule.diffconv.diffconv3 import  PDCBlock,createConvFunc
try:
...
class regionCluster(nn.Module):
    def __init__(self, dim, out_dim, proposal_w=2, proposal_h=2, fold_w=2, fold_h=2, heads=4, head_dim=24):
        ...
        self.centers_proposal = nn.AvgPool2d(2, 2)
    ...
    def forward(self, x):  # [b,c,w,h]
        if len(x.shape)!=4:
            x = rearrange(x, "b (w h) c -> b c w h", w=8)
        value = self.v(x)
        x = self.f(x)
        ...
        value2 = rearrange(value, 'b c w h -> b (w h) c')  # [B,N,D]
        # aggregate step, out shape [B,M,D]
        out = (value2.unsqueeze(dim=1) * sim.unsqueeze(dim=-1)).sum(
            dim=2)  # Different from CoC's implementation "(value2.unsqueeze(dim=1) * sim.unsqueeze(dim=-1)).sum(dim=2)", we use scatter_sum to avoid OOM.
        out = (out + value_centers) / (sim.sum(dim=-1, keepdim=True) + 1.0)

        return out, sim, w



class regionCluster2(nn.Module):
    def __init__(self, dim, out_dim, proposal_w=2, proposal_h=2, fold_w=2, fold_h=2, heads=4, head_dim=24):
        super().__init__()
        self.heads = heads
        self.head_dim = head_dim
        self.f = nn.Linear(dim, heads * head_dim)  # for similarity
        # self.proj = nn.Conv2d(heads * head_dim, out_dim, kernel_size=1)  # for projecting channel number
        self.v = nn.Linear(dim, heads * head_dim)  # for value
        self.sim_alpha = nn.Parameter(torch.ones(1))
        self.sim_beta = nn.Parameter(torch.zeros(1))
        self.centers_proposal = nn.AdaptiveAvgPool1d((2))
        # self.centers_proposal = nn.AvgPool2d(2, 2)
    ...
    def forward(self, x):  # [b,c,w,h]
        ...
        value = self.v(x).permute(0, 2, 1)
        x = self.f(x).permute(0, 2, 1)


        # 聚类过程
        centers = self.centers_proposal(x).permute(0, 2, 1) # [b,c,C_W,C_H], we set M = C_W*C_H and N = w*h
        value_centers = self.centers_proposal(value).permute(0, 2, 1)  # [b,C_W,C_H,c]
        # b, c, ww, hh = centers.shape
        sim = torch.sigmoid(
            self.sim_beta +
            self.sim_alpha * pairwise_cos_sim(
        ...
        sim = sim * mask
        # value2 = rearrange(value, 'b c w h -> b (w h) c')  # [B,N,D]
        value2 = value.permute(0, 2, 1)  # [B,N,D]
        # aggregate step, out shape [B,M,D]
        out = (value2.unsqueeze(dim=1) * sim.unsqueeze(dim=-1)).sum(
            dim=2)  # Different from CoC's implementation "(value2.unsqueeze(dim=1) * sim.unsqueeze(dim=-1)).sum(dim=2)", we use scatter_sum to avoid OOM.
        out = (out + value_centers) / (sim.sum(dim=-1, keepdim=True) + 1.0)
...
class Cluster(nn.Module):
    def __init__(self, dim, out_dim, proposal_w=2, proposal_h=2, fold_w=2, fold_h=2, heads=4, head_dim=24):
        ...
        self.head_dim = head_dim
        # self.f = nn.Conv2d(dim, heads * head_dim, kernel_size=1)  # for similarity
        self.proj = nn.Conv2d(heads * head_dim, out_dim//2, kernel_size=1)  # for projecting channel number
        self.projcd = nn.Conv2d(512, out_dim, kernel_size=1)
        self.projlast =  nn.Conv2d(512, out_dim, kernel_size=1)
        # self.v = nn.Conv2d(dim, heads * head_dim, kernel_size=1)  # for value
        # self.sim_alpha = nn.Parameter(torch.ones(1))
        # self.sim_beta = nn.Parameter(torch.zeros(1))
        # self.centers_proposal = nn.AdaptiveAvgPool2d((proposal_w, proposal_h))
        self.fold_w = fold_w
        self.fold_h = fold_h
        self.cluster16 = regionCluster(dim//2, out_dim, proposal_w, proposal_h, fold_w, fold_h, heads, head_dim)
        self.cluster4 = regionCluster(dim=head_dim, out_dim=out_dim, proposal_w=proposal_w, proposal_h=proposal_h, fold_w=fold_w, fold_h=fold_h, heads=1, head_dim=head_dim)
        self.cluster2 = regionCluster2(dim=head_dim, out_dim=out_dim, proposal_w=proposal_w, proposal_h=proposal_h,
                                      fold_w=fold_w, fold_h=fold_h, heads=1, head_dim=head_dim)
        # self.convdc = Conv2d_cd(256,256)
        self.convDC1 = PDCBlock(createConvFunc('cd'),256,256)
        self.convDC2 = PDCBlock(createConvFunc('ad'),256,256)
        # self.cluster4 = regionCluster(dim=32, out_dim=out_dim, proposal_w=proposal_w, proposal_h=proposal_h,fold_w=fold_w, fold_h=fold_h, heads=1, head_dim=head_dim)
    def forward(self, x):  # [b,c,w,h]
        # out16, sim16, w16 = self.cluster16(x)
        # out4, sim4, w4 = self.cluster4(out16)

        feat = self.projcd(x)
        x = feat[:, :256, :, :]  # First 256 channels
        cd = feat[:, 256:, :, :]  # Last 256 channels
        ...
        out16, sim16, w16 = self.cluster16(x)
        out4, sim4, _ = self.cluster4(out16)
        outcls, sim2 ,_ =self.cluster2(out4)
        # dispatch step, return to each point in a cluster

        out4_2 =(outcls.unsqueeze(dim=2) * sim2.unsqueeze(dim=-1)).sum(
            dim=1)


        out16_2 = (out4_2.unsqueeze(dim=2) * sim4.unsqueeze(dim=-1)).sum(
            dim=1)  # [B,N,D]  #将每个聚类中心对每个像素赋予权重，最后相加可以得到传播后的特征

        out = (out16_2.unsqueeze(dim=2) * sim16.unsqueeze(dim=-1)).sum(dim=1)

        out = rearrange(out, "b (w h) c -> b c w h", w=w16)
        if self.fold_w > 1 and self.fold_h > 1:
            # recover the splited regions back to big feature maps if use the region partition.
            out = rearrange(out, "(b f1 f2) c w h -> b c (f1 w) (f2 h)", f1=self.fold_w, f2=self.fold_h)
        ...
        out =self.projlast(torch.cat([out,cd],dim=1))
...
class ClusterBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_layer_scale:
            ...
            xout,outcls=self.token_mixer(self.norm1(x))
            x = x + self.drop_path(
...
if __name__ == '__main__':
    ...
    #
    # get_flops1()
    block = basic_blocks(dim=512, index=0, layers=[2, 4, 4, 4], fold_h=1, heads=1, head_dim=512)
    out = block(input)
    print(out.size())
