...
class JsonDataset(AbstractDataset):
    ...
    def _init_dataset_path(self, path):
        ...
        for record in images:
            # print(record)
            if os.path.isfile('/disk/csh/segdataset'+record[0]):
                if record[1]!='Negative':
                    tp_list.append('/disk/csh/segdataset'+record[0])
                    gt_list.append('/disk/csh/segdataset'+record[1])
                else:
                    tp_list.append('/disk/csh/segdataset' + record[0])
