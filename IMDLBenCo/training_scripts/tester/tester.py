...
import os
def save_comparison(input_img, pred, gt, save_path, index=0):
    # 如果是CUDA tensor，转到CPU
    input_img = input_img[0].cpu()
    input_img = denormalize(input_img, mean=[0.485, 0.456, 0.406],
                            std=[0.229, 0.224, 0.225])
    # input_img = torch.clamp(input_img, 0, 1)  # 避免溢出
    # input_img = TF.to_pil_image(input_img)
    pred = pred.cpu()
    gt = gt.cpu()

    # 转为 numpy
    input_img_np = TF.to_pil_image(input_img)
    ...
    plt.tight_layout()
    plt.savefig(f"{save_path}/compare_{index}.png")
    plt.close()
...
def test_one_loader(model: torch.nn.Module,
    ...
    for data_iter_step, data_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # save_comparison(data_dict['image'], mask_pred, data_dict['mask'],
        #                 '/home/csh/disk/IMDLBenCo4/previous/sparsevit/NIST16', str(data_iter_step))
        #-------------
        # save_comparison(data_dict['image'], mask_pred, data_dict['mask'],
        #                 '/home/csh/disk/segdataset/test1', str(data_iter_step))
        #-------------
        #---- Training evaluation ----
